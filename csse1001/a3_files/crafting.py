...
class GridCrafterView(tk.Frame):
    ...
    def __init__(self, master, input_size):
        ...
        # ...
        if input_size[0] == 3 and input_size[1] == 1:
            self._button_text = '==> Smelt ==>'

        else:
            self._button_text = '==> Craft ==>'

        self._input = ItemGridView(self, input_size)
        self._input.pack(side=tk.LEFT)

        self._craft_button = tk.Button(self, text=self._button_text)
    ...
    # Task 2.2 Crafting: You may add additional methods here
        ...
        self._input.bind(event,
                         lambda e: callback(self._input.xy_to_grid((e.x, e.y)),
                                            e))
...
class CraftingWindow(tk.Toplevel):
    ...
    def __init__(self, master, title, hot_bar: Grid, inventory: Grid, crafter: GridCrafter):
        ...
        Parameters:
            ...
            inventory (Grid): The inventory to show above the hotbar, below the
            crafting widget
        ...
        for widget_key in ('inventory', 'hot_bar'):
            ...
            view_widget.bind_for_id("<Button-1>",
                                    lambda key, e, widget_key=widget_key:
                                    self._handle_left_click(widget_key, key, e))
            view_widget.bind_for_id("<Button-2>",
                                    lambda key, e, widget_key=widget_key:
                                    self._handle_right_click(widget_key, key, e))
    ...
    def _load_crafter_view(self):
        ...
        self._source_views['crafter'] = crafter_view = \
            GridCrafterView(self, self._sources['crafter'].get_input_size())
    ...
    def redraw(self):
        ...
        selected_widget, selected_position = self._selection if self._selection\
            else (None, None)

        for key, widget in self._sources.items():
            view_widget = self._source_views[key]
            view_widget.render(widget.items(), selected_position if
            selected_widget == key else None)
            ...
            if selection == self._selection:
                ...
                if to_stack is None:
                    ...
                    if to_stack.matches(from_stack) and from_stack.get_item().\
                        ...
                        to_stack.absorb(from_stack, maximum=
                        None if 'ctrl' in key_modifiers else 1)
                    else:
