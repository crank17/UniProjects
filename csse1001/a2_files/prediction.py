...
# Definition of WeatherPrediction Super class
...
class SimplePrediction(WeatherPrediction):
    ...
    def high_temperature(self):
        ...
        weather_items = self._weather_data.get_data(self._number_days)
        # loop iterating over high temp to find highest value
        for weather_item in weather_items:
    ...
    def low_temperature(self):
        ...
        weather_items = self._weather_data.get_data(self._number_days)
        # loop iterating over low temp to find lowest value
        for weather_item in weather_items:
    ...
    def humidity(self):
        ...
        av_humidity = 0
        weather_items = self._weather_data.get_data(self._number_days)
        # simple average loop, used throughout SimplePrediction and SophisticatedPrediction
        for weather_item in weather_items:
...
class SophisticatedPrediction(WeatherPrediction):
    ...
    def __init__(self, weather_data, number_days):
        ...
        """
        super().__init__(weather_data)
        # if number of days entered exceeds the length of the list, set it to equal the number of days.
        if(number_days > weather_data.size()):
            self._number_days = weather_data.size()
        else:
            self._number_days = number_days
    ...
    def humidity(self):
        ...
        # set range for average humidity
    ...
    def cloud_cover(self):
        ...
        if self._yesterdays_air_pressure < self.get_average_pressure():
            cloud_cover += 2
        # 9 oktas is the maximum cloud coverage
        if cloud_cover > 9:
            cloud_cover = 9
